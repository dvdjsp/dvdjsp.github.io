...
or if you had a general model with $$M$$ parameters $$\theta_1,\theta_2,\dots,\theta_M$$ that takes in multiple inputs $$\boldsymbol{x}=(x_1,x_2,\dots, x_p)$$ given by $$f(\boldsymbol{x}; \boldsymbol{\theta})$$ that we can gather as an array or vector commonly denoted with a bold $$\boldsymbol{\theta}$$
...
```
where we **represent** the standard error of the mean for each value $$y_i$$ as its error bars.
