...
class IsingModel {
  constructor(adjacencyMatrix) {
    ...
    this.state = Array(this.numNodes).fill().map((_, i) => i % 2 === 0 ? 1 : -1 // Alternating pattern to ensure good distribution
    );
...
function StaticIsingView({
  ...
  useEffect(() => {
    ...
    const g = svg.append('g').attr('class', 'static-container');
    ...
    g.selectAll('.static-link').data(links).enter().append('line').attr('class', 'static-link').attr('x1', d => d.source.x).attr('y1', d => d.source.y).attr('x2', d => d.target.x).attr('y2', d => d.target.y).attr('stroke', '#999').attr('stroke-width', 1);
    ...
    const nodeData = positions.map((pos, i) => ({
      ...pos,
      nodeId: i,
      // Store the original index for mapping to spin array
      spin: spins[i] || 0 // Initial spin state
    }));
    g.selectAll('.static-node').data(nodeData).enter().append('circle').attr('class', 'static-node').attr('r', 8).attr('cx', d => d.x).attr('cy', d => d.y).attr('data-node-id', d => d.nodeId) // Store index as data attribute
    .attr('stroke', '#333').attr('stroke-width', 1).attr('fill', d => d.spin === 1 ? 'black' : 'white'); // Initial color
  ...
  useEffect(() => {
    ...
    d3.select(svgRef.current).selectAll('.static-node').attr('fill', function () {
      ...
      if (spin === 1) blackCount++;else whiteCount++;
...
function GraphPreview({
  ...
  const toggleStretch = () => {
    ...
    simulation.force('link').distance(linkDistance).strength(springStrength);
  ...
  const shakeGraph = () => {
    ...
    simulation.force('link').distance(d => originalLinkDistance * 3) // 3x the original distance
    ...
    shakingTimerRef.current = setTimeout(() => {
      if (simulationRef.current) {
        ...
        simulation.force('link').distance(originalLinkDistance).strength(originalLinkStrength);
  ...
  const fitToScreen = () => {
    ...
    const translate = [(width - graphWidth * scale) / 2 - minX * scale, (height - graphHeight * scale) / 2 - minY * scale];

    // Apply the transform with transition
    svg.transition().duration(500).call(d3.zoom().transform, d3.zoomIdentity.translate(translate[0], translate[1]).scale(scale * 0.95) // Slightly smaller to ensure everything fits
    );
  };
  useEffect(() => {
    ...
    // Create the links
    const link = g.selectAll('.link').data(links).join('line').attr('class', 'link').attr('stroke', '#999').attr('stroke-width', 1);

    // Create the nodes with correct colors based on spins if available
    const node = g.selectAll('.node').data(nodes).join('circle').attr('class', 'node').attr('r', nodeRadius).attr('fill', (d, i) => spins && i < spins.length ? spins[i] === 1 ? 'black' : 'white' : '#000').attr('stroke', '#333').attr('stroke-width', 1);
    ...
    const simulation = d3.forceSimulation(nodes).force('link', d3.forceLink(links).id(d => d.id).distance(linkDistance).strength(springStrength)).force('charge', d3.forceManyBody().strength(forceStrength)).force('center', d3.forceCenter(virtualWidth / 2, virtualHeight / 2)).velocityDecay(0.4) // Slightly higher damping to prevent excessive oscillation
    ...
    simulation.on('tick', () => {
      ...
      node.attr('cx', d => d.x = Math.max(nodeRadius, Math.min(virtualWidth - nodeRadius, d.x))).attr('cy', d => d.y = Math.max(nodeRadius, Math.min(virtualHeight - nodeRadius, d.y)));
    });
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "flex mt-2 justify-between flex-wrap gap-2"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: toggleStretch,
    className: `text-sm py-2 px-3 rounded font-bold ${isStretched ? 'bg-red-500 text-white' : 'bg-blue-600 text-white'}`
  }, isStretched ? 'Freeze Graph' : 'Stretch Graph'), /*#__PURE__*/React.createElement("button", {
    onClick: shakeGraph,
    disabled: isShaking,
    className: `text-sm py-2 px-3 rounded font-bold ${isShaking ? 'bg-purple-300 text-white cursor-not-allowed' : 'bg-purple-600 text-white hover:bg-purple-700'}`
  }, isShaking ? 'Untangling...' : 'Untangle Graph!'), /*#__PURE__*/React.createElement("button", {
    onClick: toggleContinuousSimulation,
    className: `text-sm py-2 px-3 rounded font-bold ${continuousSimulation ? 'bg-green-600 text-white' : 'bg-yellow-500 text-white'}`
  }, continuousSimulation ? 'Continuous: ON' : 'Continuous: OFF'), /*#__PURE__*/React.createElement("button", {
    onClick: stopStretch,
    className: "text-sm py-2 px-3 rounded bg-red-600 text-white font-bold"
  }, "Stop Stretch"), /*#__PURE__*/React.createElement("button", {
    onClick: fitToScreen,
    className: "text-sm py-2 px-3 rounded bg-gray-600 text-white font-bold"
  }, "Fit to Screen")), /*#__PURE__*/React.createElement("div", {
    className: "text-xs text-gray-500 text-center"
  }, "Virtual boundary: ", displayScaleFactor, "x screen size (", nodeCount, " nodes) |", isShaking ? " Untangling in progress... " : "", continuousSimulation ? " Simulation will run continuously" : " Simulation will stop when stable")));
}
...
function MagnetizationCalculator() {
  ...
  const estimateCriticalTemperature = async matrix => {
    ...
    for (let i = 1; i < results.length; i++) {
      const slope = (results[i - 1].magnetization - results[i].magnetization) / (results[i].temperature - results[i - 1].temperature);
      if (slope > maxSlope) {
        maxSlope = slope;
  ...
  const generateTemperaturePoints = (initialEstimate = null) => {
    ...
    return Array.from({
      length: tempPoints
    }, (_, i) => parseFloat((tempMax - i * step).toFixed(3)));
  };
  ...
  const calculateMagnetizationAtTemperature = async (temperature, model) => {
    try {
      ...
      const meanEnergy = energyValues.length > 0 ? energyValues.reduce((sum, val) => sum + val, 0) / energyValues.length : null;
      return {
        mean: meanMag,
        stdDev: stdDev,
  ...
  const startCalculation = async () => {
    ...
    try {
      ...
      const temperatures = generateTemperaturePoints(firstRunComplete && useTwoStageSampling ? initialTcEstimate : null);
      const results = [];
      if (firstRunComplete && useTwoStageSampling) {
      ...
      for (let i = 0; i < temperatures.length; i++) {
        ...
        if (firstRunComplete && useTwoStageSampling && magnetizationData.some(d => Math.abs(d.temperature - temp) < 0.01)) {
  ...
  const estimateCriticalTemperatureFromData = data => {
    ...
    for (let i = 1; i < smoothedData.length; i++) {
      ...
      const slope = (smoothedData[i - 1].magnetization - smoothedData[i].magnetization) / (smoothedData[i].temperature - smoothedData[i - 1].temperature);
    ...
    if (critTempIndex >= 1 && critTempIndex < derivatives.length - 1) {
      ...
      const x = [derivatives[critTempIndex - 1].temperature, derivatives[critTempIndex].temperature, derivatives[critTempIndex + 1].temperature];
      const y = [derivatives[critTempIndex - 1].slope, derivatives[critTempIndex].slope, derivatives[critTempIndex + 1].slope];
      try {
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col w-full bg-white p-4 rounded-lg"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-xl font-bold mb-4 text-center"
  }, "Ising Model on Arbitrary Graphs"), /*#__PURE__*/React.createElement("div", {
  ...
  }, "Select Lattice Type"), /*#__PURE__*/React.createElement("div", {
    className: "flex flex-wrap gap-2"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => loadExampleGraph('grid'),
    className: "p-2 border rounded bg-blue-500 text-white hover:bg-blue-600 flex-1",
    disabled: isCalculating
  }, "Square Grid"), /*#__PURE__*/React.createElement("button", {
    onClick: () => loadExampleGraph('triangular'),
    className: "p-2 border rounded bg-blue-500 text-white hover:bg-blue-600 flex-1",
    disabled: isCalculating
  }, "Triangular"), /*#__PURE__*/React.createElement("button", {
    onClick: () => loadExampleGraph('hexagonal'),
    className: "p-2 border rounded bg-blue-500 text-white hover:bg-blue-600 flex-1",
    disabled: isCalculating
  }, "Hexagonal"))), /*#__PURE__*/React.createElement("div", {
    className: "flex-1"
  }, /*#__PURE__*/React.createElement("h3", {
    className: "font-bold mb-2"
  }, "Upload Custom Matrix"), /*#__PURE__*/React.createElement("input", {
    type: "file",
    accept: ".csv",
  ...
  }, "CSV format: row,column,value (Mathematica sparse array format)"))))), fileError && /*#__PURE__*/React.createElement("div", {
    className: "mb-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative"
  }, /*#__PURE__*/React.createElement("strong", {
    className: "font-bold"
  }, "Error:"), /*#__PURE__*/React.createElement("span", {
    className: "block sm:inline"
  }, " ", fileError), /*#__PURE__*/React.createElement("button", {
    className: "absolute top-0 bottom-0 right-0 px-4 py-3",
    onClick: () => setFileError(null)
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-xl"
  }, "\xD7"))), isGraphLoaded && /*#__PURE__*/React.createElement("div", {
    className: "bg-gray-100 p-3 rounded-lg mb-4"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col gap-4"
  }, /*#__PURE__*/React.createElement("div", {
    className: "w-full bg-white rounded shadow-md"
  }, !showSimulationView ? /*#__PURE__*/React.createElement(GraphPreview, {
    adjacencyMatrix: adjacencyMatrix,
    spins: spins,
    onSvgRef: handleSvgRef
  }) : /*#__PURE__*/React.createElement(StaticIsingView, {
    adjacencyMatrix: adjacencyMatrix,
    spins: spins,
    positions: nodePositions
  })), /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-2 md:grid-cols-4 gap-4 p-2 bg-white rounded"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    className: "font-semibold"
  }, "Nodes:"), " ", graphStats.nodes), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    className: "font-semibold"
  }, "Edges:"), " ", graphStats.edges), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    className: "font-semibold"
  }, "Average Degree:"), " ", (2 * graphStats.edges / graphStats.nodes).toFixed(2)), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    className: "font-semibold"
  }, "Connectivity:"), " ", (graphStats.edges / (graphStats.nodes * (graphStats.nodes - 1) / 2) * 100).toFixed(2), "%")))), isGraphLoaded && /*#__PURE__*/React.createElement("div", {
    className: "bg-gray-100 p-3 rounded-lg mb-4"
  }, /*#__PURE__*/React.createElement("h3", {
    className: "font-bold mb-2"
  }, "Real-time Ising Model Simulation"), /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col md:flex-row gap-4"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex-1"
  }, /*#__PURE__*/React.createElement("h4", {
    className: "font-semibold mb-1"
  }, "Temperature Control"), /*#__PURE__*/React.createElement("div", {
  ...
  }, simulationTemp.toFixed(1))), /*#__PURE__*/React.createElement("div", {
    className: "text-xs text-gray-600 flex justify-between mt-1"
  }, /*#__PURE__*/React.createElement("span", null, "Cold (Ordered)"), /*#__PURE__*/React.createElement("span", null, "Hot (Disordered)")), /*#__PURE__*/React.createElement("div", {
    className: "mt-4"
  }, /*#__PURE__*/React.createElement("h4", {
    className: "font-semibold mb-1"
  }, "Time Scale"), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-2"
  }, /*#__PURE__*/React.createElement("input", {
    type: "range",
    min: "50",
    ...
    className: "font-bold text-lg w-16 text-right"
  }, timeScale)), /*#__PURE__*/React.createElement("div", {
    className: "text-xs text-gray-600 flex justify-between mt-1"
  }, /*#__PURE__*/React.createElement("span", null, "Slower"), /*#__PURE__*/React.createElement("span", null, "Faster"))), /*#__PURE__*/React.createElement("div", {
    className: "mt-3"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-sm"
  }, /*#__PURE__*/React.createElement("span", {
    className: "font-semibold"
  }, "Critical Temperature:"), " ", criticalTemp ? criticalTemp.toFixed(2) : "Estimating..."), /*#__PURE__*/React.createElement("div", {
    className: "text-sm"
  }, /*#__PURE__*/React.createElement("span", {
    className: "font-semibold"
  }, "Current Phase:"), " ", getPhaseDescription()))), /*#__PURE__*/React.createElement("div", {
    className: "flex-1"
  }, /*#__PURE__*/React.createElement("h4", {
    className: "font-semibold mb-2"
  }, "Simulation Statistics"), /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-2 gap-2"
  }, /*#__PURE__*/React.createElement("div", {
    className: "text-sm"
  }, /*#__PURE__*/React.createElement("span", {
    className: "font-semibold"
  }, "Magnetization:"), " ", magnetization.toFixed(3)), /*#__PURE__*/React.createElement("div", {
    className: "text-sm"
  }, /*#__PURE__*/React.createElement("span", {
    className: "font-semibold"
  }, "|Magnetization|:"), " ", absoluteMagnetization.toFixed(3)), /*#__PURE__*/React.createElement("div", {
    className: "text-sm"
  }, /*#__PURE__*/React.createElement("span", {
    className: "font-semibold"
  }, "Energy:"), " ", energy.toFixed(3)), /*#__PURE__*/React.createElement("div", {
    className: "text-sm"
  }, /*#__PURE__*/React.createElement("span", {
    className: "font-semibold"
  }, "Energy per Site:"), " ", (energy / (graphStats.nodes || 1)).toFixed(3))))), /*#__PURE__*/React.createElement("div", {
    ...
    disabled: isCalculating
  }, isRealTimeSimulating ? 'Stop Simulation' : 'Simulate Magnet'), /*#__PURE__*/React.createElement("button", {
    onClick: resetSpins,
    className: "px-4 py-2 font-bold bg-yellow-500 text-white rounded hover:bg-yellow-600",
    disabled: isCalculating
  }, "Randomize Spins"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setAllSpins(1),
    className: "px-4 py-2 font-bold bg-black text-white rounded hover:bg-gray-800",
    disabled: isCalculating
  }, "All Up (Black)"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setAllSpins(-1),
    className: "px-4 py-2 font-bold bg-white text-black border-2 border-black rounded hover:bg-gray-100",
    disabled: isCalculating
  }, "All Down (White)"))), /*#__PURE__*/React.createElement("div", {
    className: "bg-gray-100 p-3 rounded-lg mb-4"
  }, /*#__PURE__*/React.createElement("h3", {
    className: "font-bold mb-2"
  }, "Magnetization Curve Calculation"), /*#__PURE__*/React.createElement("div", {
    className: "mb-3"
  }, /*#__PURE__*/React.createElement("h4", {
    className: "font-semibold mb-1"
  }, "Temperature Range"), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-2"
  }, /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "0.1",
    ...
    className: "font-semibold mb-1"
  }, "Equilibration Steps:"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "1000",
    max: "100000",
    step: "1000",
    ...
    className: "text-xs text-gray-500 mt-1"
  }, "Steps to reach equilibrium (higher = more accurate, slower calculation)")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h4", {
    className: "font-semibold mb-1"
  }, "Measurement Steps:"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "100",
    max: "20000",
    step: "100",
    ...
    className: "text-xs text-gray-500 mt-1"
  }, "Steps between measurements (higher = better decorrelation)")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h4", {
    className: "font-semibold mb-1"
  }, "Measurement Trials:"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "10",
    max: "1000",
    step: "10",
    ...
    className: "text-xs text-gray-500 mt-1"
  }, "Number of measurements averaged per temperature point")))), /*#__PURE__*/React.createElement("div", {
    className: "flex justify-center mb-4"
  }, !isCalculating ? /*#__PURE__*/React.createElement("button", {
    onClick: startCalculation,
    className: "px-6 py-2 bg-blue-500 text-white rounded hover:bg-blue-600",
    disabled: !isGraphLoaded || isRealTimeSimulating
  }, "Calculate Magnetization Curve") : /*#__PURE__*/React.createElement("button", {
    onClick: stopCalculation,
    className: "px-6 py-2 bg-red-500 text-white rounded hover:bg-red-600"
  }, "Stop Calculation")), isCalculating && /*#__PURE__*/React.createElement("div", {
    className: "mb-4"
  }, /*#__PURE__*/React.createElement("div", {
  ...
  }, "Calculating... ", Math.round(progress), "% complete", firstRunComplete && useTwoStageSampling && initialTcEstimate && /*#__PURE__*/React.createElement("span", null, " (refining around Tc\u2248", initialTcEstimate.toFixed(2), ")"))), magnetizationData.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-lg shadow-md p-4 mb-4"
  }, /*#__PURE__*/React.createElement("h3", {
    className: "font-bold mb-3"
  }, "Magnetization vs Temperature"), criticalTemp && /*#__PURE__*/React.createElement("div", {
    className: "mb-3 p-2 bg-yellow-100 rounded"
  }, /*#__PURE__*/React.createElement("span", null, "Estimated critical temperature: T", /*#__PURE__*/React.createElement("sub", null, "c"), " = ", criticalTemp.toFixed(2), " \xB1 ", criticalTemp ? "0.05" : "?")), /*#__PURE__*/React.createElement("div", {
    className: "h-96"
    ...
    connectNulls: false
  }), criticalTemp && /*#__PURE__*/React.createElement(ReferenceLine, {
    x: criticalTemp,
    stroke: "red",
    strokeDasharray: "5 5",
    label: {
  ...
  })))), calculationTime > 0 && /*#__PURE__*/React.createElement("div", {
    className: "text-sm text-gray-600 text-right mt-2"
  }, "Calculation time: ", calculationTime.toFixed(1), " seconds")), /*#__PURE__*/React.createElement("div", {
    className: "text-sm text-gray-600 bg-gray-100 p-3 rounded-lg"
  }, /*#__PURE__*/React.createElement("h3", {
    className: "font-bold mb-2"
  }, "About the Ising Model"), /*#__PURE__*/React.createElement("p", null, "This tool combines interactive real-time simulation with quantitative magnetization curve calculation for the Ising model on arbitrary graph structures. The Ising model is a mathematical model of ferromagnetism in statistical mechanics."), /*#__PURE__*/React.createElement("p", {
    className: "mt-2"
  }, /*#__PURE__*/React.createElement("strong", null, "Controls:")), /*#__PURE__*/React.createElement("ul", {
    className: "list-disc pl-5 mt-1"
  }, /*#__PURE__*/React.createElement("li", null, /*#__PURE__*/React.createElement("strong", null, "Simulate Magnet"), " - Run real-time Monte Carlo simulation at current temperature"), /*#__PURE__*/React.createElement("li", null, /*#__PURE__*/React.createElement("strong", null, "Time Scale"), " - Control simulation speed (50-500 Monte Carlo steps per frame)"), /*#__PURE__*/React.createElement("li", null, /*#__PURE__*/React.createElement("strong", null, "Calculate Magnetization Curve"), " - Generate full magnetization vs. temperature curve")), /*#__PURE__*/React.createElement("p", {
    className: "mt-2"
